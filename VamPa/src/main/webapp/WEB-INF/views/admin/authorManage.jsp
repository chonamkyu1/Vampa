...
<script>
	$(document).ready(function(){
		let result = '<c:out value="${enroll_result}"/>';  // 작가 등록 컨트롤러
		let mresult = '<c:out value="${modify_result}"/>';
		
		checkResult(result);
		ckeckmResult(mresult);
	...
	$(".pageMaker_btn a").on("click", function(e) {
		e.preventDefault();  // a태그나 submit 태그는 누르게 되면 href를 통해 이동하거나, 창이 새로고침하여 실행된다. 이거를 방지
	...
	$("#searchForm button").on("click", function(e){
		...
		// -> 변경이유는 만약 7페이지에 있는 상태에서 검색을 하는 경우 검색어로 필터링된 7페이지로 이동, 하지만 필터링된 검색 결과가 7페이지 존재하지 않을 경우 문제가 됨
		//		그래서 1페이지로 이동
