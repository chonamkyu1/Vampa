...
<script>
	...
	let pwckCheck = false;		// 비밀번호 확인
	let pwckcorCheck = false;	// 비빌번호 일치 확인
...
// 				console.log("data : " , data);
				checkBox.attr("disabled", false); // disabled 속성 풀기
	...
	function execution_daum_address() {
		new daum.Postcode({
			oncomplete: function(data) {
                ...
                // $("[name=memberAddr1]").val(data.zonecode); // 대체 가능
                $(".address_input_2").val(addr);
                // $("[name=memberAddr2]").val(addr); // 대체가능
                
                // 상세주소 입력란 disabled 속성 변경 및 커서를 상세주소 필드로 이동한다.
				$(".address_input_3").attr("readonly", false); // readonly 속성 풀어주기
